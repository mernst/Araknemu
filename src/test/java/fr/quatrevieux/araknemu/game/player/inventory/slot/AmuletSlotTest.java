...
class AmuletSlotTest extends GameBaseCase {
    private AmuletSlot slot;
    ...
    public void setUp() throws Exception {
        ...
        slot = new AmuletSlot(new DefaultListenerAggregate(), Mockito.mock(ItemStorage.class), gamePlayer());
    }
    ...
    void checkBadQuantity() throws ContainerException {
        assertThrows(InventoryException.class, () -> slot.check(container.get(ItemService.class).create(2425), 10));
    }
    ...
    void checkBadLevel() throws ContainerException, SQLException {
        ...
        assertThrows(BadLevelException.class, () -> slot.check(container.get(ItemService.class).create(112425), 1));
    }

    void checkSuccess() throws ContainerException, InventoryException {
        slot.check(container.get(ItemService.class).create(2425), 1);
    }
