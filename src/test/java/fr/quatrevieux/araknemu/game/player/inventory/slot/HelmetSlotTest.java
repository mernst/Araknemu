...
class HelmetSlotTest extends GameBaseCase {
    private HelmetSlot slot;
    ...
    public void setUp() throws Exception {
        ...
        slot = new HelmetSlot(new DefaultListenerAggregate(), Mockito.mock(ItemStorage.class), gamePlayer());
    }
    ...
    void checkBadQuantity() throws ContainerException {
        assertThrows(InventoryException.class, () -> slot.check(container.get(ItemService.class).create(2411), 10));
    }
    ...
    void checkBadLevel() throws ContainerException, SQLException {
        ...
        assertThrows(BadLevelException.class, () -> slot.check(container.get(ItemService.class).create(112411), 1));
    }

    void checkSuccess() throws ContainerException, InventoryException {
        slot.check(container.get(ItemService.class).create(2411), 1);
    }
    ...
    void setSuccess() throws ContainerException, InventoryException {
        InventoryEntry entry = new InventoryEntry(null, new PlayerItem(1, 1, 2411, new ArrayList<>(), 1, -1), container.get(ItemService.class).create(2411));
