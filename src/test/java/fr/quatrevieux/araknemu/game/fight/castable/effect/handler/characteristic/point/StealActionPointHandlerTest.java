...
class StealActionPointHandlerTest extends FightBaseCase {
    ...
    void withArea() {
        ...
        Optional<Buff> buff0 = fight.fighters().get(0).buffs().stream().filter(b -> b.effect().effect() == 111).findFirst();
        Optional<Buff> buff1 = fight.fighters().get(1).buffs().stream().filter(b -> b.effect().effect() == 101).findFirst();
        Optional<Buff> buff2 = fight.fighters().get(2).buffs().stream().filter(b -> b.effect().effect() == 101).findFirst();
